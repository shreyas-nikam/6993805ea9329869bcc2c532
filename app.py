...
# Main Header
st.title("QuLab: Lab 51: Robo-Advisor Simulation")
st.divider()
...
if 'glide_preds' not in st.session_state:
    st.session_state.glide_preds = None
if 'ml_models' not in st.session_state:
    st.session_state.ml_models = None
if 'segment_profile_df' not in st.session_state:
    st.session_state.segment_profile_df = None
if 'SEGMENT_NAMES' not in st.session_state:
    st.session_state.SEGMENT_NAMES = None
if 'per_asset_mae_df' not in st.session_state:
    st.session_state.per_asset_mae_df = None
...
if 'segments_calculated' not in st.session_state:
    st.session_state.segments_calculated = False
if 'allocations_predicted' not in st.session_state:
    st.session_state.allocations_predicted = False
if 'suitability_checked' not in st.session_state:
...
# Using index to maintain state consistency with selectbox
try:
...
elif st.session_state.current_page == "2. Client Segmentation":
...
elif st.session_state.current_page == "5. Client Reporting":
...
elif st.session_state.current_page == "6. Suitability Monitoring & Synthesis":
